...
gate: it refuses unless both isolation flags are on *and* the backend is allow-listed.
This build does not open a Grok Bot or any other long-lived VM. Approved skill
state is never written from this path. There is no Policy.external_trajectory_import flag.
"""
...
def external_computer_handler(inputs: dict, workdir: Path) -> ToolResult:
    ...
    if not isolation.long_lived_computer_backend:
        reasons.append("isolation.long_lived_computer_backend is false")
